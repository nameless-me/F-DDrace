...
void CShop::BuyItem(int ClientID, int Item)
{
	...
	if (IsType(HOUSE_SHOP))
	{
		...
		case ITEM_NULL:			GameServer()->SendChatTarget(ClientID, "You bought a null item. Congratulations on doing absolutely nothing!"); break;
		case ITEM_SHOTGUN:			pAccount->m_SpawnWeapon[0] += Amount; break;
		case ITEM_GRENADE:			pAccount->m_SpawnWeapon[1] += Amount; break;
		case ITEM_RIFLE:				pAccount->m_SpawnWeapon[2] += Amount; break;
		}
	}
	else if (IsType(HOUSE_PLOT_SHOP))
	{
